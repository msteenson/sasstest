...
$(function () {
  ...
  QUnit.test('should provide no conflict', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof($.fn.dropdown), 'undefined', 'dropdown was set back to undefined (org value)');
  });
  ...
  QUnit.test('should not open dropdown if escape key was pressed on the toggle', function (assert) {
    ...
    $(dropdownHTML).appendTo('#qunit-fixture');
    var $dropdown = $('#qunit-fixture').find('[data-toggle="dropdown"]').bootstrapDropdown();
    var $button = $('button[data-toggle="dropdown"]'); // Key escape
  ...
  QUnit.test('should close dropdown and set focus back to toggle when escape is pressed while focused on a dropdown item', function (assert) {
    ...
    $dropdown.parent('.dropdown').on('shown.bs.dropdown', function () {
      ...
      $item[0].focus();
      assert.ok($(document.activeElement)[0] === $item[0], 'menu item initial focus set'); // Key escape
  ...
  QUnit.test('should ignore keyboard events for <input>s and <textarea>s within dropdown-menu, except for escape key', function (assert) {
    ...
    $dropdown.parent('.dropdown').on('shown.bs.dropdown', function () {
      ...
      assert.ok($(document.activeElement)[0] === $textarea[0], 'textarea still focused'); // Key up
      ...
      assert.ok($(document.activeElement)[0] === $textarea[0], 'textarea still focused'); // Key down
      ...
      assert.ok($(document.activeElement)[0] === $textarea[0], 'textarea still focused'); // Key escape
  ...
  QUnit.test('should ignore space key events for <input>s within dropdown, and accept up, down and escape', function (assert) {
    ...
    $dropdown.parent('.dropdown').one('shown.bs.dropdown', function () {
      assert.ok(true, 'shown was fired'); // Key space

      $input.trigger('focus').trigger($.Event('keydown', {
        which: 32
      }));
      assert.ok($dropdown.parent('.dropdown').hasClass('show'), 'dropdown menu is shown');
      assert.ok($(document.activeElement).is($input), 'input is still focused'); // Key escape
  ...
  QUnit.test('should ignore space key events for <textarea>s within dropdown, and accept up, down and escape', function (assert) {
    ...
    $dropdown.parent('.dropdown').one('shown.bs.dropdown', function () {
      ...
      assert.ok($dropdown.parent('.dropdown').hasClass('show'), 'dropdown menu is shown');
      assert.ok($(document.activeElement).is($textarea), 'textarea is still focused'); // Key escape
  ...
  QUnit.test('should create offset modifier correctly when offset option is a function', function (assert) {
    ...
    var getOffset = function getOffset(offsets) {
      return offsets;
    };
