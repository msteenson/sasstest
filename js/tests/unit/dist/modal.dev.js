...
$(function () {
  ...
  QUnit.module('modal', {
    before: function before() {
      ...
      $('<style type="text/css"> .modal-scrollbar-measure { position: absolute; top: -9999px; width: 50px; height: 50px; overflow: scroll; } </style>').appendTo('head'); // Function to calculate the scrollbar width which is then compared to the padding or margin changes

      $.fn.getScrollbarWidth = $.fn.modal.Constructor.prototype._getScrollbarWidth; // Simulate scrollbars
  ...
  QUnit.test('should provide no conflict', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof($.fn.modal), 'undefined', 'modal was set back to undefined (orig value)');
  });
  ...
  QUnit.test('should store the original body padding in data-padding-right before showing', function (assert) {
    ...
    $('<div id="modal-test"/>').on('hidden.bs.modal', function () {
      assert.strictEqual(_typeof($body.data('padding-right')), 'undefined', 'data-padding-right should be cleared after closing');
      $body.removeAttr('style');
      done();
    }).on('shown.bs.modal', function () {
  ...
  QUnit.test('should not adjust the inline body padding when it does not overflow', function (assert) {
    ...
    var done = assert.async();
    var $body = $(document.body);
    var originalPadding = $body.css('padding-right'); // Hide scrollbars to prevent the body overflowing
    ...
    $('<div id="modal-test"/>').on('shown.bs.modal', function () {
      ...
      $(this).bootstrapModal('hide'); // Restore scrollbars
  ...
  QUnit.test('should store the original padding of fixed elements in data-padding-right before showing', function (assert) {
    ...
    $('<div id="modal-test"/>').on('hidden.bs.modal', function () {
      assert.strictEqual(_typeof($element.data('padding-right')), 'undefined', 'data-padding-right should be cleared after closing');
      $element.remove();
      done();
    }).on('shown.bs.modal', function () {
  ...
  QUnit.test('should store the original margin of sticky elements in data-margin-right before showing', function (assert) {
    ...
    $('<div id="modal-test"/>').on('hidden.bs.modal', function () {
      assert.strictEqual(_typeof($element.data('margin-right')), 'undefined', 'data-margin-right should be cleared after closing');
      $element.remove();
      done();
    }).on('shown.bs.modal', function () {
  ...
  QUnit.test('should not execute js from target', function (assert) {
    assert.expect(0);
    var done = assert.async(); // This toggle button contains XSS payload in its data-target
    // Note: it uses the onerror handler of an img element to execute the js, because a simple script element does not work here
    //       a script element works in manual tests though, so here it is likely blocked by the qunit framework

    var $toggleBtn = $('<button data-toggle="modal" data-target="&lt;div&gt;&lt;image src=&quot;missing.png&quot; onerror=&quot;$(&apos;#qunit-fixture button.control&apos;).trigger(&apos;click&apos;)&quot;&gt;&lt;/div&gt;"/>').appendTo('#qunit-fixture'); // The XSS payload above does not have a closure over this function and cannot access the assert object directly
  ...
  QUnit.test('should not adjust the inline body padding when it does not overflow, even on a scaled display', function (assert) {
    ...
    var done = assert.async();
    var $modal = $(['<div id="modal-test">', '  <div class="modal-dialog">', '    <div class="modal-content">', '      <div class="modal-body" />', '    </div>', '  </div>', '</div>'].join('')).appendTo('#qunit-fixture');
    var originalPadding = window.getComputedStyle(document.body).paddingRight; // Remove body margins as would be done by Bootstrap css

    document.body.style.margin = '0'; // Hide scrollbars to prevent the body overflowing

    document.body.style.overflow = 'hidden'; // Simulate a discrepancy between exact, i.e. floating point body width, and rounded body width
    ...
    $modal.on('shown.bs.modal', function () {
      ...
      assert.strictEqual(currentPadding, originalPadding, 'body padding should not be adjusted'); // Restore overridden css
