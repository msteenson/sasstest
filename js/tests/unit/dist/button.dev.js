...
$(function () {
  ...
  QUnit.test('should provide no conflict', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof($.fn.button), 'undefined', 'button was set back to undefined (org value)');
  });
  ...
  QUnit.test('should toggle aria-pressed on buttons with container', function (assert) {
    assert.expect(1);
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<button id="btn1" class="btn btn-secondary" type="button">One</button>' + '<button class="btn btn-secondary" type="button">Two</button>' + '</div>';
  ...
  QUnit.test('should assign active class on page load to button checkbox with checked attribute', function (assert) {
    assert.expect(1);
    var done = assert.async();
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn btn-primary">' + '<input type="checkbox" id="radio" checked> Checkbox' + '</label>' + '</div>';
  ...
  QUnit.test('should remove active class on page load from button checkbox without checked attribute', function (assert) {
    assert.expect(1);
    var done = assert.async();
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn btn-primary active">' + '<input type="checkbox" id="radio"> Checkbox' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $btn = $group.children().eq(0);
    $(window).trigger($.Event('load'));
    setTimeout(function () {
  ...
  QUnit.test('should trigger input change event when toggled button has input field', function (assert) {
    assert.expect(1);
    var done = assert.async();
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn btn-primary">' + '<input type="radio" id="radio">Radio' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
  ...
  QUnit.test('should fire click event on input', function (assert) {
    assert.expect(1);
    var done = assert.async();
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn btn-primary active">' + '<input type="checkbox" id="option1"> Option 1' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $btn = $group.children().eq(0);
  ...
  QUnit.test('should not add aria-pressed on labels for radio/checkbox inputs in a data-toggle="buttons" group', function (assert) {
    assert.expect(2);
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn btn-primary"><input type="checkbox"> Checkbox</label>' + '<label class="btn btn-primary"><input type="radio" name="options"> Radio</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $btn1 = $group.children().eq(0);
    var $btn2 = $group.children().eq(1);
  ...
  QUnit.test('should handle disabled attribute on non-button elements', function (assert) {
    assert.expect(4);
    var groupHTML = '<div class="btn-group disabled" data-toggle="buttons" aria-disabled="true" disabled>' + '<label class="btn btn-danger disabled">' + '<input type="checkbox" aria-disabled="true" disabled>' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $btn = $group.children().eq(0);
  ...
  QUnit.test('should not set active class if inner hidden checkbox is disabled but author forgot to set disabled class on outer button', function (assert) {
    assert.expect(4);
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn btn-danger">' + '<input type="checkbox" disabled>' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $btn = $group.children().eq(0);
    var $input = $btn.children().eq(0);
    assert.ok($btn.is(':not(.active)'), 'button is initially not active');
  ...
  QUnit.test('should correctly set checked state on input and active class on label when using <label><input></label> structure', function (assert) {
    assert.expect(4);
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn">' + '<input type="checkbox">' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
  ...
  QUnit.test('should correctly set checked state on input and active class on the faked button when using <div><input></div> structure', function (assert) {
    assert.expect(4);
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<div class="btn">' + '<input type="checkbox" aria-label="Check">' + '</div>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $btn = $group.children().eq(0);
    var $input = $btn.children().eq(0);
  ...
  QUnit.test('should not try and set checked property on an input of type="hidden"', function (assert) {
    assert.expect(2);
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn">' + '<input type="hidden">' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $label = $group.children().eq(0);
    var $input = $label.children().eq(0);
  ...
  QUnit.test('should not try and set checked property on an input that is not a radio button or checkbox', function (assert) {
    assert.expect(2);
    var groupHTML = '<div class="btn-group" data-toggle="buttons">' + '<label class="btn">' + '<input type="text">' + '</label>' + '</div>';
    var $group = $(groupHTML).appendTo('#qunit-fixture');
    var $label = $group.children().eq(0);
    var $input = $label.children().eq(0);
