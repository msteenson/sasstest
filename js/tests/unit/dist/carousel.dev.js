...
$(function () {
  ...
  var stylesCarousel = ['<style>', '  .carousel.pointer-event { -ms-touch-action: none; touch-action: none; }', '</style>'].join('');
  ...
  QUnit.test('should provide no conflict', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof($.fn.carousel), 'undefined', 'carousel was set back to undefined (orig value)');
  });
  QUnit.test('should return version', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof(Carousel.VERSION), 'string');
  });
