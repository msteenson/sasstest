...
$(function () {
  ...
  QUnit.test('should provide no conflict', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof($.fn.tooltip), 'undefined', 'tooltip was set back to undefined (org value)');
  });
  ...
  QUnit.test('should destroy tooltip', function (assert) {
    assert.expect(7);
    var $tooltip = $('<div/>').bootstrapTooltip().on('click.foo', function () {}); // eslint-disable-line no-empty-function
  ...
  QUnit.test('should do nothing when an attempt is made to hide an uninitialized tooltip', function (assert) {
    ...
    assert.strictEqual(_typeof($tooltip.data('bs.tooltip')), 'undefined', 'should not initialize the tooltip');
  });
  ...
  QUnit.test('should allow passing a custom function for sanitization', function (assert) {
    ...
    var $trigger = $('<a href="#" rel="tooltip" data-trigger="click" title="Another tooltip"/>').appendTo('#qunit-fixture').bootstrapTooltip({
      ...
      sanitizeFn: function sanitizeFn(input) {
        return input;
      }
    });
    var tooltip = $trigger.data('bs.tooltip');
