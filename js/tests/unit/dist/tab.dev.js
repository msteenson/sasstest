...
$(function () {
  ...
  QUnit.test('should provide no conflict', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof($.fn.tab), 'undefined', 'tab was set back to undefined (org value)');
  });
  ...
  QUnit.test('should activate element by tab id', function (assert) {
    assert.expect(2);
    var tabsHTML = '<ul class="nav">' + '<li><a href="#home">Home</a></li>' + '<li><a href="#profile">Profile</a></li>' + '</ul>';
  ...
  QUnit.test('should activate element by tab id', function (assert) {
    assert.expect(2);
    var pillsHTML = '<ul class="nav nav-pills">' + '<li><a href="#home">Home</a></li>' + '<li><a href="#profile">Profile</a></li>' + '</ul>';
    $('<ul><li id="home"/><li id="profile"/></ul>').appendTo('#qunit-fixture');
  ...
  QUnit.test('should activate element by tab id in ordered list', function (assert) {
    assert.expect(2);
    var pillsHTML = '<ol class="nav nav-pills">' + '<li><a href="#home">Home</a></li>' + '<li><a href="#profile">Profile</a></li>' + '</ol>';
  ...
  QUnit.test('should not fire shown when show is prevented', function (assert) {
    ...
    $('<div class="nav"/>').on('show.bs.tab', function (e) {
      e.preventDefault();
  ...
  QUnit.test('should not fire shown when tab is already active', function (assert) {
    assert.expect(0);
    var tabsHTML = '<ul class="nav nav-tabs" role="tablist">' + '<li class="nav-item" role="presentation"><a href="#home" class="nav-link active" role="tab">Home</a></li>' + '<li class="nav-item" role="presentation"><a href="#profile" class="nav-link" role="tab">Profile</a></li>' + '</ul>' + '<div class="tab-content">' + '<div class="tab-pane active" id="home" role="tabpanel"></div>' + '<div class="tab-pane" id="profile" role="tabpanel"></div>' + '</div>';
    $(tabsHTML).find('a.active').on('shown.bs.tab', function () {
      assert.ok(true, 'shown event fired');
    }).bootstrapTab('show');
  });
  QUnit.test('should not fire shown when tab is disabled', function (assert) {
    assert.expect(0);
    var tabsHTML = '<ul class="nav nav-tabs" role="tablist">' + '<li class="nav-item"><a href="#home" class="nav-link active" role="tab">Home</a></li>' + '<li class="nav-item"><a href="#profile" class="nav-link disabled" role="tab">Profile</a></li>' + '</ul>' + '<div class="tab-content">' + '<div class="tab-pane active" id="home" role="tabpanel"></div>' + '<div class="tab-pane" id="profile" role="tabpanel"></div>' + '</div>';
    $(tabsHTML).find('a.disabled').on('shown.bs.tab', function () {
      assert.ok(true, 'shown event fired');
    }).bootstrapTab('show');
  });
  QUnit.test('show and shown events should reference correct relatedTarget', function (assert) {
    assert.expect(2);
    var done = assert.async();
    var dropHTML = '<ul class="drop nav">' + '  <li class="dropdown"><a data-toggle="dropdown" href="#">1</a>' + '    <ul class="dropdown-menu nav">' + '      <li><a href="#a1-1" data-toggle="tab">1-1</a></li>' + '      <li><a href="#a1-2" data-toggle="tab">1-2</a></li>' + '    </ul>' + '  </li>' + '</ul>';
    $(dropHTML).find('ul > li:first-child a').bootstrapTab('show').end().find('ul > li:last-child a').on('show.bs.tab', function (e) {
  ...
  QUnit.test('should fire hide and hidden events', function (assert) {
    ...
    var done = assert.async();
    var tabsHTML = '<ul class="nav">' + '<li><a href="#home">Home</a></li>' + '<li><a href="#profile">Profile</a></li>' + '</ul>';
    $(tabsHTML).find('li:first-child a').on('hide.bs.tab', function () {
      assert.ok(true, 'hide event fired');
    }).bootstrapTab('show').end().find('li:last-child a').bootstrapTab('show');
    $(tabsHTML).find('li:first-child a').on('hidden.bs.tab', function () {
  ...
  QUnit.test('should not fire hidden when hide is prevented', function (assert) {
    ...
    var done = assert.async();
    var tabsHTML = '<ul class="nav">' + '<li><a href="#home">Home</a></li>' + '<li><a href="#profile">Profile</a></li>' + '</ul>';
    $(tabsHTML).find('li:first-child a').on('hide.bs.tab', function (e) {
      e.preventDefault();
      assert.ok(true, 'hide event fired');
      done();
  ...
  QUnit.test('selected tab should have aria-selected', function (assert) {
    assert.expect(8);
    var tabsHTML = '<ul class="nav nav-tabs">' + '<li><a class="nav-item active" href="#home" toggle="tab" aria-selected="true">Home</a></li>' + '<li><a class="nav-item" href="#profile" toggle="tab" aria-selected="false">Profile</a></li>' + '</ul>';
  ...
  QUnit.test('selected tab should deactivate previous selected tab', function (assert) {
    assert.expect(2);
    var tabsHTML = '<ul class="nav nav-tabs">' + '<li class="nav-item"><a class="nav-link active" href="#home" data-toggle="tab">Home</a></li>' + '<li class="nav-item"><a class="nav-link" href="#profile" data-toggle="tab">Profile</a></li>' + '</ul>';
    var $tabs = $(tabsHTML).appendTo('#qunit-fixture');
    $tabs.find('li:last-child a').trigger('click');
  ...
  QUnit.test('should handle removed tabs', function (assert) {
    assert.expect(1);
    var done = assert.async();
    var html = ['<ul class="nav nav-tabs" role="tablist">', '  <li class="nav-item" role="presentation">', '    <a class="nav-link nav-tab" href="#profile" role="tab" data-toggle="tab">', '      <button class="close"><span aria-hidden="true">&times;</span></button>', '    </a>', '  </li>', '  <li class="nav-item" role="presentation">', '    <a id="secondNav" class="nav-link nav-tab" href="#buzz" role="tab" data-toggle="tab">', '      <button class="close"><span aria-hidden="true">&times;</span></button>', '    </a>', '  </li>', '  <li class="nav-item" role="presentation">', '    <a class="nav-link nav-tab" href="#references" role="tab" data-toggle="tab">', '      <button id="btnClose" class="close"><span aria-hidden="true">&times;</span></button>', '    </a>', '  </li>', '</ul>', '<div class="tab-content">', '  <div role="tabpanel" class="tab-pane fade show active" id="profile">test 1</div>', '  <div role="tabpanel" class="tab-pane fade" id="buzz">test 2</div>', '  <div role="tabpanel" class="tab-pane fade" id="references">test 3</div>', '</div>'].join('');
  ...
  QUnit.test('should not add show class to tab panes if there is no `.fade` class', function (assert) {
    assert.expect(1);
    var done = assert.async();
    var html = ['<ul class="nav nav-tabs" role="tablist">', '  <li class="nav-item" role="presentation">', '    <a class="nav-link nav-tab" href="#home" role="tab" data-toggle="tab">Home</a>', '  </li>', '  <li class="nav-item" role="presentation">', '    <a id="secondNav" class="nav-link nav-tab" href="#profile" role="tab" data-toggle="tab">Profile</a>', '  </li>', '</ul>', '<div class="tab-content" role="presentation">', '  <div role="tabpanel" class="tab-pane" id="home">test 1</div>', '  <div role="tabpanel" class="tab-pane" id="profile">test 2</div>', '</div>'].join('');
    $(html).appendTo('#qunit-fixture');
    $('#secondNav').on('shown.bs.tab', function () {
