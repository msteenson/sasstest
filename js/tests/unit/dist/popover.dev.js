...
$(function () {
  ...
  QUnit.test('should provide no conflict', function (assert) {
    assert.expect(1);
    assert.strictEqual(_typeof($.fn.popover), 'undefined', 'popover was set back to undefined (org value)');
  });
  ...
  QUnit.test('should allow DOMElement title and content (html: true)', function (assert) {
    ...
    assert.ok($.contains($('.popover').get(0), title), 'title node moved, not copied'); // toLowerCase because IE8 will return <I>...</I>
  ...
  QUnit.test('should do nothing when an attempt is made to hide an uninitialized popover', function (assert) {
    ...
    assert.strictEqual(_typeof($popover.data('bs.popover')), 'undefined', 'should not initialize the popover');
  });
